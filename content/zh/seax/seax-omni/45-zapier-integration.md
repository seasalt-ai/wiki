...
1. **測試動作：** 測試 Agentic Send 步驟以確認輸出正確。
2. **執行 Zap：** 啟動工作流程。Agentic Send 是「在許多非常複雜事件中救命又省時的工具」，因為它能減少設定錯誤並大幅簡化流程。

---

## 為什麼要使用 Agentic Send？

| 功能 | 傳統 Zapier 設定 | Seasalt.ai Agentic Send |
| :--- | :--- | :--- |
| **複雜度** | 高（5 個以上步驟） | 低（2 個步驟） |
| **資料擷取** | 需要「格式化器」步驟 | AI 自動擷取資料 |
| **錯誤處理** | 格式錯誤時容易中斷 | AI 自動修正格式（例如加上 +1 國碼） |
| **邏輯判斷** | 需要「路徑」（If/Then） | AI 自動判斷號碼是否存在 |

---

## 常見使用案例

**Agentic Send** 功能不受觸發器限制，意味著它可與 Zapier 上的任何 7000+ 應用程式搭配使用。

* **Google Sheets：** 偵測新的一列（例如藥局訂單），並解析該列以發送取貨通知。
* **Typeform / Jotform：** 在潛在客戶提交聯絡表單後立即發送簡訊，即使他們的電話號碼格式怪異。
* **Mailchimp：** 當使用者被加入特定電子郵件清單時觸發簡訊通知。