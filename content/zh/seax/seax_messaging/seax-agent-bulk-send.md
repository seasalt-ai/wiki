...
要將 SeaX 與 SeaChat 整合，您需要一個電話號碼來發送訊息，以及一個 AI 助理來處理對話。您可以在 SeaX 的 **Workspace** 下的 **Numbers** 部分找到電話號碼，並在 SeaChat 的 **Workspace** 下的 **Agents** 部分找到 AI 助理。
...
在群發訊息的 **Recipients** 部分可以查看所有標籤，並選擇用於群發訊息的標籤。
...
選擇收件人標籤後，您將進入 **Contacts** 部分。在這裡，您可以查看聯絡人姓名、電話號碼及其關聯的標籤。取消勾選您不希望包含在訊息中的聯絡人，然後繼續到 **Compose** 部分。
...
創建活動時，請提供一個名稱並選擇關聯的標籤。注意，在 **Compose** 頁面的標籤是用於在活動開始後新增到所選聯絡人的。如果希望向特定聯絡人發送訊息，需在 **Recipients** 和 **Contacts** 部分選擇相應標籤。

#### 外發 SMS 和電話

設置活動名稱和標籤後，選擇執行方法。選擇 **Chat/Voice Agent**，讓 AI 助理處理外發訊息。
...
>
> 點擊 SeaX 中所選助理旁的 **Configure AI Agent** 按鈕，在 SeaChat 平台上配置助理。
>
> 如果無法看到助理，表示 AI 助理尚未在 SeaChat 上設置。點擊 **Configure AI Agent** 設置助理，並記得保存設置。
...
> 額外設置 - 捕捉收件人按鍵和點擊追蹤
>
> SeaX 會追踪收件人與 AI 助理的互動。啟用 **Capture Recipients' Key Presses** 記錄通話過程中的按鍵輸入，用於調查。
>
> 啟用 **Click Tracking** 追踪 SMS 中的點擊，例如鏈接點擊追踪。

### 發送者

選擇活動的發送者，並選擇發送訊息的電話號碼。

如需新增電話號碼，點擊 **Quote for a New Number**，SeaSalt 將提供報價。
...
點擊 **Send Now** 激活活動，開始通過 SeaX 發送訊息。在側欄的 **Campaigns** 部分監控活動進展。
...
- 所有由 SeaX 通過 SeaChat 助理啟動的外發通話都會顯示在 SeaChat 和 SeaX 的 **Conversations** 部分。然而，如果從 SeaChat 接收到來電，則不會同步至 SeaX。
